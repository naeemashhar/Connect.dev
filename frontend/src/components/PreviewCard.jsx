...
const PreviewCard = ({ user }) => {
  ...
  return (
    <div className="max-w-md w-full bg-white/40 dark:bg-black/30 px-6 py-13 rounded-3xl shadow-xl text-center border border-cyan-700 dark:border-cyan-500 transition-all duration-300 hover:scale-[1.01]">
      {/* Profile Image */}
      ...
      {/* Name + Title */}
      <h2 className="text-2xl font-semibold tracking-tight text-[#021431] dark:text-white flex items-center justify-center gap-1 flex-wrap">
        {firstName} {lastName}
        {user.isPremium && (
          <Verified
        ...
        <span className="text-sm text-cyan-700 dark:text-cyan-400 font-light ml-2">
          ({title || "No Title"})
        </span>
      </h2>
      ...
      <div className="mt-3 flex flex-wrap justify-center gap-3 text-sm text-gray-600 dark:text-white/60">
        <span className="flex items-center gap-1">
          <i className="ri-map-pin-line text-cyan-600 dark:text-cyan-400" />{" "}
          {city}, {country}
        </span>
        <span className="flex items-center gap-1">
          <i className="ri-user-line text-cyan-600 dark:text-cyan-400" />{" "}
          {gender}
        </span>
        <span className="flex items-center gap-1">
          <i className="ri-calendar-line text-cyan-600 dark:text-cyan-400" />{" "}
          {age} yrs
        </span>
      </div>
